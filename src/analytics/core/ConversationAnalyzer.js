...
class ConversationAnalyzer {
  ...
  async loadConversations(stateCalculator) {
    ...
    try {
      // Search for .jsonl files only in the projects subdirectory (where conversations live)
        ...
        try {
          ...
          for (const item of items) {
            ...
            try {
              ...
              console.warn(chalk.yellow(`Skipping inaccessible item: ${itemPath}`));
            }
          }
        } catch (dirError) {
          console.warn(chalk.yellow(`Cannot read directory: ${dir}`));
        }

        return files;
      };

      // Only search in projects directory - that's where conversations are stored
      const projectsDir = path.join(this.claudeDir, 'projects');
      const jsonlFiles = await fs.pathExists(projectsDir)
        ? await findJsonlFiles(projectsDir)
        : [];
  ...
  parseAndCorrelateToolMessages(lines) {
    ...
    const toolResultIds = new Set(); // Track tool_result entries to skip
    ...
    // Third pass: convert to standard format, skipping standalone tool_result entries
    const processedMessages = [];

    for (const item of entries) {
      // Skip tool_result entries that were attached to a tool_use
      if (toolResultIds.has(item)) {
        continue;
      }
